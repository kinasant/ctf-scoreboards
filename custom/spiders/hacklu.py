...
#table.xpath(td[1]//a/@href").get()
    ...
    def parse(self, response):
        ...
        team = []
        name = []
        tables = response.xpath('//div/table/tbody/tr')
        for table in tables[1:]:
            y =  table.xpath('td[5]/text()').get()
            if int(y)<=0:
                break
            points.append(y)
            name.append(table.xpath('td[3]//a/text()').get().strip())
            team.append(table.xpath("td[2]/a/@href").get())
        df['points'] = points
        df['name'] = name
        url = ""
        challenges = []
        ...
        for i in team:
            ...
            next_page = url+i
            request= yield scrapy.Request(next_page,headers=self.headers,dont_filter=True)
            s = self.parse_team(request)
            challenges.append(s)
            cnt+=1
        df["challenge"] = challenges
        df.to_csv(Path(__file__).parent / 'hacklu.csv',index=False)
    def parse_team(self,response):
        table = response.xpath('//div[@id="challenges"]/table/tbody/tr')[1:]
        challenge_name = [i.strip() for i in table.xpath('td/a/text()').getall()]
        challenge_category =table.xpath('td[3]//text()').getall()
        challenge_points = [i.strip() for i in table.xpath('td[4]//text()').getall()]
