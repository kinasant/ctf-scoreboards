...
class TeamSpider(scrapy.Spider): 
    ...
    def start_requests(self):
        urls = [
            'https://nhnc.ic3dt3a.org/login?next=%2Fscoreboard%3F',
            ]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.login)
        yield scrapy.Request(url=r'file://No%20Hack%20No%20CTF.html',callback=self.parse)
...
#table.xpath(td[1]//a/@href").get()
    def login(self,response):
        ...
        yield FormRequest.from_response(response,formdata={'name':'{REDACTED}','password':"{I_didnt_leave_the_password_here_this_time}",'nonce':nonce},dont_filter=False)
    @inline_requests
    def parse(self, response):
        ...
        df.to_csv(Path(__file__).parent / 'nohacknoctf.csv',index=False)
