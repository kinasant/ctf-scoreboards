...
class TeamSpider(scrapy.Spider): 
    ...
    def start_requests(self):
        urls = [
            'https://ctfnhack.ctflib.eu/login?next=%2Fscoreboard%3F',
            ]
        for url in urls:
...
#table.xpath(td[1]//a/@href").get()
    ...
    def parse(self, response):
        ...
        name = []
        self.log(response)
        tables = response.xpath("//table[@x-data='UsersTable']/tbody/tr")
        for table in tables:
            y =  table.xpath("td[3]/text()").get()
            #if int(y)<=50:
                #break
            points.append(y)
            name.append(table.xpath("td[2]//a/text()").get().strip())
            team.append(table.xpath("td[2]//a/@href").get())
        df["Position"] = range(1,len(team)+1)
        df['points'] = points
        df['name'] = name
        url = "https://ctfnhack.ctflib.eu"
        challenges = []
        cnt = 0
        for i in team:
        ...
        df.to_csv(Path(__file__).parent / 'NTUA.csv',index=False)
