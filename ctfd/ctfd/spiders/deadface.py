...
class TeamSpider(scrapy.Spider): 
    ...
    def start_requests(self):
        urls = [
            'https://ctf.deadface.io/login?next=%2Fscoreboard%3F',
            ]
        for url in urls:
...
#table.xpath(td[1]//a/@href").get()
    def login(self,response):
        ...
        yield FormRequest.from_response(response,formdata={'name':'temp1','password':"temp",'nonce':nonce},callback=self.parse,dont_filter=True)
    @inline_requests
    def parse(self, response):
        ...
        df.to_csv(Path(__file__).parent / 'deadface.csv',index=False)
