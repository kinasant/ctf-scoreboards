...
#table.xpath(td[1]//a/@href").get()
    def login(self,response):
        ...
        yield FormRequest.from_response(response,formnumber=1,formdata={'name':'temp1','password':"temp",'nonce':nonce},callback=self.parse,dont_filter=True)
    def parse_file(self, response):
        ...
        self.team = team
        self.points = points
        yield scrapy.Request(url='https://ctf.jerseyctf.com/login?next=%2Fscoreboard%3F',callback = self.login)
    @inline_requests
